...
static NSInteger nIndicatorLenNumeral[]  = {10, 12, 14};
static NSInteger nIndicatorLenAlphabet[] = { 9, 11, 13};
static NSInteger nIndicatorLen8Bit[]	   = { 8, 16, 16};
static NSInteger nIndicatorLenKanji[]	   = { 8, 10, 12};


/////////////////////////////////////////////////////////////////////////////
// QR_Encode クラスの構築/消滅

@implementation CQR_Encode
...
-(BOOL) EncodeData:(NSInteger) nLevel nVersion:(NSInteger) nVersion bAutoExtent:(BOOL) bAutoExtent nMaskingNo:(NSInteger)nMaskingNo lpsSource:(LPCSTR) lpsSource ncSource: (NSInteger) ncSource
{
	...
	// バージョン(型番)チェック
    NSInteger nEncodeVersion = [self GetEncodeVersion:nVersion lpsSource:lpsSource ncLength:ncLength];
	...
	// ターミネータコード"0000"付加
	NSInteger ncDataCodeWord = QR_VersonInfo[m_nVersion].ncDataCodeWord[nLevel];

    NSInteger ncTerminater = [self min:4 b:(ncDataCodeWord * 8) - m_ncDataCodeWordBit];

	if (ncTerminater > 0)
        m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:0 ncData:ncTerminater];
	...
	NSInteger nDataCwIndex = 0; // データコードワード処理位置

	// データブロック分割数
	NSInteger ncBlock1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[nLevel].ncRSBlock;
	NSInteger ncBlock2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[nLevel].ncRSBlock;
	NSInteger ncBlockSum = ncBlock1 + ncBlock2;

	NSInteger nBlockNo = 0; // 処理中ブロック番号

	// ブロック別データコードワード数
	NSInteger ncDataCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[nLevel].ncDataCodeWord;
	NSInteger ncDataCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[nLevel].ncDataCodeWord;
	...
	NSInteger ncRSCw1 = QR_VersonInfo[m_nVersion].RS_BlockInfo1[nLevel].ncAllCodeWord - ncDataCw1;
	NSInteger ncRSCw2 = QR_VersonInfo[m_nVersion].RS_BlockInfo2[nLevel].ncAllCodeWord - ncDataCw2;
	...
	for (i = 0; i < ncBlock1; ++i)
	{
        ...
        [self GetRSCodeWord:m_byRSWork ncDataCodeWord:ncDataCw1 ncRSCodeWord:ncRSCw1];
	...
	for (i = 0; i < ncBlock2; ++i)
	{
        ...
        [self GetRSCodeWord:m_byRSWork ncDataCodeWord:ncDataCw2 ncRSCodeWord:ncRSCw2];
...
{
	NSInteger nVerGroup = nVersion >= 27 ? QR_VRESION_L : (nVersion >= 10 ? QR_VRESION_M : QR_VRESION_S);
	NSInteger i, j;

	for (i = nVerGroup; i <= QR_VRESION_L; ++i)
	{
		if ([self EncodeSourceData:lpsSource ncLength:ncLength nVerGroup:i])
		{
			if (i == QR_VRESION_S)
			{
				for (j = 1; j <= 9; ++j)
...
-(BOOL) EncodeSourceData:(LPCSTR) lpsSource ncLength:(NSInteger) ncLength nVerGroup:(NSInteger) nVerGroup
{
	...
	for (m_ncDataBlock = i = 0; i < ncLength; ++i)
	{
		...
		if (i < ncLength - 1 && [self IsKanjiData:lpsSource[i] c2:lpsSource[i + 1]])
			byMode = QR_MODE_KANJI;
		else if ([self IsNumeralData:lpsSource[i]])
			byMode = QR_MODE_NUMERAL;
		else if ([self IsAlphabetData:lpsSource[i]])
	...
	NSInteger ncSrcBits, ncDstBits; // 元のビット長と単一の英数字モードブロック化した場合のビット長

	NSInteger nBlock = 0;

	while (nBlock < m_ncDataBlock - 1)
	{
		NSInteger ncJoinFront, ncJoinBehind; // 前後８ビットバイトモードブロックと結合した場合のビット長
		NSInteger nJoinPosition = 0; // ８ビットバイトモードブロックとの結合：-1=前と結合、0=結合しない、1=後ろと結合
			...
			(m_byBlockMode[nBlock] == QR_MODE_ALPHABET && m_byBlockMode[nBlock + 1] == QR_MODE_NUMERAL))
		{
            ...
            ncSrcBits = [self GetBitLength:m_byBlockMode[nBlock] ncData:m_nBlockLength[nBlock] nVerGroup:nVerGroup] + [self GetBitLength:m_byBlockMode[nBlock + 1] ncData:m_nBlockLength[nBlock + 1] nVerGroup:nVerGroup];
			
            ncDstBits = [self GetBitLength:QR_MODE_ALPHABET ncData:m_nBlockLength[nBlock] + m_nBlockLength[nBlock + 1] nVerGroup:nVerGroup];
			...
			if (ncSrcBits > ncDstBits)
			{
				...
				{
					// 前に８ビットバイトモードブロックあり                    
                    ncJoinFront = [self GetBitLength:QR_MODE_8BIT ncData:m_nBlockLength[nBlock - 1] + m_nBlockLength[nBlock] nVerGroup:nVerGroup] +
                    [self GetBitLength:m_byBlockMode[nBlock + 1] ncData:m_nBlockLength[nBlock + 1] nVerGroup:nVerGroup];

                    
					if (ncJoinFront > ncDstBits + [self GetBitLength:QR_MODE_8BIT ncData:m_nBlockLength[nBlock - 1] nVerGroup:nVerGroup])
				...
				{
					// 後ろに８ビットバイトモードブロックあり
                    ncJoinBehind = [self GetBitLength:m_byBlockMode[nBlock] ncData:m_nBlockLength[nBlock] nVerGroup:nVerGroup] +
                    [self GetBitLength:QR_MODE_8BIT ncData:m_nBlockLength[nBlock + 1] + m_nBlockLength[nBlock + 2] nVerGroup:nVerGroup];
                    
					if (ncJoinBehind > ncDstBits + [self GetBitLength:QR_MODE_8BIT ncData:m_nBlockLength[nBlock + 2] nVerGroup:nVerGroup])
	...
	while (nBlock < m_ncDataBlock - 1)
	{
        ncSrcBits = [self GetBitLength:m_byBlockMode[nBlock] ncData:m_nBlockLength[nBlock] nVerGroup:nVerGroup]
        + [self GetBitLength:m_byBlockMode[nBlock + 1] ncData:m_nBlockLength[nBlock + 1] nVerGroup:nVerGroup];
        
        ncDstBits = [self GetBitLength:QR_MODE_8BIT ncData:m_nBlockLength[nBlock] + m_nBlockLength[nBlock + 1] nVerGroup:nVerGroup];
	...
	/////////////////////////////////////////////////////////////////////////
	// ビット配列化
	NSInteger ncComplete = 0; // 処理済データカウンタ
	WORD wBinCode;
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		if (m_byBlockMode[i] == QR_MODE_NUMERAL)
		{
			...
			// インジケータ(0001b)
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:1 ncData:4];

			// 文字数セット
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:(WORD)m_nBlockLength[i] ncData:nIndicatorLenNumeral[nVerGroup]];
			...
			for (j = 0; j < m_nBlockLength[i]; j += 3)
			{
				if (j < m_nBlockLength[i] - 2)
				{
                    ...
                    m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:wBinCode ncData:10];
				}
				else if (j == m_nBlockLength[i] - 2)
				{
                    ...
                    m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:wBinCode ncData:7];
				}
				else if (j == m_nBlockLength[i] - 1)
				{
                    ...
                    m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:wBinCode ncData:4];
				}
			}
		...
		{ 
			...
			// モードインジケータ(0010b)
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:2 ncData:4];

			// 文字数セット
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:(WORD)m_nBlockLength[i] ncData:nIndicatorLenAlphabet[nVerGroup]];
			...
			for (j = 0; j < m_nBlockLength[i]; j += 2)
			{
				...
				{
                    wBinCode = (WORD)(([self AlphabetToBinaly:lpsSource[ncComplete + j]] * 45) +
                                      [self AlphabetToBinaly:lpsSource[ncComplete + j + 1]]);

                    m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:wBinCode ncData:11];
				}
				else
				{
					// 端数１バイト
                    wBinCode = (WORD)[self AlphabetToBinaly:lpsSource[ncComplete + j]];

                    m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:wBinCode ncData:6];
				}
			}
		...
		else if (m_byBlockMode[i] == QR_MODE_8BIT)
		{
			...
			// モードインジケータ(0100b)
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:4 ncData:4];
            
			// 文字数セット
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:(WORD)m_nBlockLength[i] ncData:nIndicatorLen8Bit[nVerGroup]];
            
			// ビット列保存
			for (j = 0; j < m_nBlockLength[i]; ++j)
			{
                m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:(WORD)lpsSource[ncComplete + j] ncData:8];
			}
		...
		else // m_byBlockMode[i] == QR_MODE_KANJI
		{
			...
			// モードインジケータ(1000b)
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:8 ncData:4];

			// 文字数セット
            m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:(WORD)(m_nBlockLength[i] / 2) ncData:nIndicatorLenKanji[nVerGroup]];

			// 漢字モードでビット列保存
			for (j = 0; j < m_nBlockLength[i] / 2; ++j)
			{
                WORD wBinCode = [self KanjiToBinaly:(WORD)(((BYTE)lpsSource[ncComplete + (j * 2)] << 8) + (BYTE)lpsSource[ncComplete + (j * 2) + 1])];

                m_ncDataCodeWordBit = [self SetBitStream:m_ncDataCodeWordBit wData:wBinCode ncData:13];
			}
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			...
			for (j = 0; j < ncRSCodeWord; ++j)
			{
				...
				BYTE nExpElement = (BYTE)(((NSInteger)(byRSExp[ncRSCodeWord][j] + nExpFirst)) % 255);
...
-(void) FormatModule
{
	...
	if (m_nMaskingNo == -1)
	{
        ...
        [self SetMaskingPattern:m_nMaskingNo];
        [self SetFormatInfoPattern:m_nMaskingNo];

		NSInteger nMinPenalty = [self CountPenalty];

		for (i = 1; i <= 7; ++i)
		{
            [self SetMaskingPattern:i];
            [self SetFormatInfoPattern:i];

			NSInteger nPenalty = [self CountPenalty];
...
-(void) SetFunctionModule
{
	...
	// 位置検出パターン
    [self SetFinderPattern:0 y:0];
    [self SetFinderPattern:m_nSymbleSize - 7 y:0];
    [self SetFinderPattern:0 y: m_nSymbleSize - 7];
	...
	for (i = 0; i < QR_VersonInfo[m_nVersion].ncAlignPoint; ++i)
	{
        ...
        [self SetAlignmentPattern:QR_VersonInfo[m_nVersion].nAlignPoint[i] y:6];
        [self SetAlignmentPattern:6 y:QR_VersonInfo[m_nVersion].nAlignPoint[i]];

		for (j = 0; j < QR_VersonInfo[m_nVersion].ncAlignPoint; ++j)
		{
            [self SetAlignmentPattern:QR_VersonInfo[m_nVersion].nAlignPoint[i] y:QR_VersonInfo[m_nVersion].nAlignPoint[j]];
		}
	}
...
-(void) SetVersionPattern
{
	...
	NSInteger nVerData = m_nVersion << 12;
...
{
	NSInteger x = m_nSymbleSize;
	NSInteger y = m_nSymbleSize - 1;

	NSInteger nCoef_x = 1; // ｘ軸配置向き
	NSInteger nCoef_y = 1; // ｙ軸配置向き
...
-(void) SetFormatInfoPattern:(NSInteger) nPatternNo
{
	...
	NSInteger nFormatData = nFormatInfo << 10;
